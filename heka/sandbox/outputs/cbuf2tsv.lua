...
--[[
Convert a circular buffer output to a TSV for non-Heka dashboard consumption

Config:
    output_path = path to write the converted cbuf(s) to
...
.. code-block:: ini
    ...
    [CbufToDashboard]
    type = "SandboxOutput"
    filename = "cbuf_dashboard.lua"
    message_matcher = "Type == 'heka.sandbox-output' && Fields[payload_type] == 'cbuf'" # convert all cbufs
    ticker_interval = 60

        [CbufToDashboard.config]
        output_path = "/tmp"
